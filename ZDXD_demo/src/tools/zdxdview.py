...
from .config import ZDXDVIEW_MODEL, ZHIPU_AI_KEY
...
def map_response(img: GeneratedImage):
    return ToolObservation(
        content_type='image',
        text='zdxdview 已经生成并向用户展示了生成的图片。',
        image_url=img.url,
        role_metadata='zdxdview_result'
    )

def tool_call(prompt: str, session_id: str) -> list[ToolObservation]:
    client = get_zhipu_client()
    response = client.images.generations(model=ZDXDVIEW_MODEL, prompt=prompt).data
